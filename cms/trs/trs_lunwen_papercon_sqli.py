...
class trs_lunwen_papercon_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if time.time() - start_time >= 6:
                printGreen("[+]Success:存在TRS学位论文系统papercon处SQL注入漏洞...(高危)\tpayload: "+vulnurl+"\npost: "+json.dumps(post_data, indent=4))
            else:
                printBlue("[-]Info:不存在trs_lunwen_papercon_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = trs_lunwen_papercon_sqli(sys.argv[1])
    testVuln.run()
