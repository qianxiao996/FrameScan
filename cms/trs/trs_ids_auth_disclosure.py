    ...
    def run(self):
        ...
        try:
            ...
            if r"JavaHome" in req.text and r"java.runtime.name" in req.text and r"java.vm.version" in req.text:
                printGreen("[+]Success:存在TRS ids身份认证信息泄露漏洞...(中危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在trs_ids_auth_disclosure漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = trs_ids_auth_disclosure(sys.argv[1])
    testVuln.run()
