...
class uniportal_bypass_priv_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if req.status_code == 200 and r"<a href=SurveyStatisShow.jsp" in req.text:
                printGreen("[+]Success:存在东软UniPortal1.2未授权访问漏洞...(高危)\tpayload: "+vulnurl)
            ...
            if r"ShowText.jsp" in req1.text and r"ShowText.jsp" not in req2.text:
                printGreen("[+]Success:存在东软UniPortal1.2 SQL注入漏洞...(高危)\tpayload: "+falseurl)
            else:
                printBlue("[-]Info:不存在uniportal_bypass_priv_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = uniportal_bypass_priv_sqli(sys.argv[1])
    testVuln.run()