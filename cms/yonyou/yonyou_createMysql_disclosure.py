...
class yonyou_createMysql_disclosure:
        ...
        try:
            for payload in payloads:
                ...
                if r"root" in req.text or r"localhost" in req.text:
                    printGreen("[+]Success:存在用友致远A6协同系统数据库账号泄露...(高危)\tpayload: "+vulnurl)
                else:
                    printBlue("[-]Info:不存在yonyou_createMysql_disclosure漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = yonyou_createMysql_disclosure(sys.argv[1])
    testVuln.run()
