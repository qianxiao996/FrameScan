...
class yonyou_initData_disclosure:
    ...
    def run(self):
        ...
        try:
            ...
            if r"personList" in req.text and r"new Person" in req.text:
                printGreen("[+]Success:存在用友致远A6协同系统敏感信息泄露漏洞...(敏感信息)\tpayload: "+vulnurl)
            ...
            if time.time() - start_time >= 6:
                printGreen("[+]Success:存在用友致远A6协同系统SQL注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在yonyou_initData_disclosure漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = yonyou_initData_disclosure(sys.argv[1])
    testVuln.run()
