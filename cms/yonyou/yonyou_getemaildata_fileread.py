...
class yonyou_getemaildata_fileread:
    ...
    def run(self):
        ...
        try:
            ...
            if req.status_code == 200 and r"patch" in req.text:
                printGreen("[+]Success:存在用友CRM系统任意文件读取漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在yonyou_getemaildata_fileread漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = yonyou_getemaildata_fileread(sys.argv[1])
    testVuln.run()