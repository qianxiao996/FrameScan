        ...
        for turl in [r"/web/icc/chat/chat?c=1&s=1",
            ...
            try:
                ...
                if r"Active Internet connections" in req.text:
                    printGreen("[+]Success:存在用友struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Linux]")

                if r"Active Connections" in req.text or r"活动连接" in req.text:
                    printGreen("[+]Success:存在用友struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Windows]")

                if r"LISTEN" in req.text:
                    cprint("[+]可能存在用友struts 命令执行漏洞...(高危)\tpayload: "+vulnurl)
                else:
                    printBlue("[-]Info:不存在yonyou_icc_struts2漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = yonyou_icc_struts2(sys.argv[1])
    testVuln.run()
