...
class yonyou_ehr_resetpwd_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if time.time() - start_time >= 6:
                printGreen("[+]Success:存在用友EHR系统 ResetPwd.jsp SQL注入漏洞...(高危)\tpayload: "+vulnurl+"\npost: "+json.dumps(post_data, indent=4))

            else:
                printBlue("[-]Info:不存在yonyou_ehr_resetpwd_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = yonyou_ehr_resetpwd_sqli(sys.argv[1])
    testVuln.run()