    ...
    def run(self):
        ...
        try:
            ...
            if r"81dc9bdb52d04dc20036dbd8313ed055" in req.text:
                printGreen("[+]Success:存在xplus MYSQL通用注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在xplus_mysql_mssql_sqli漏洞")
            ...
            if r"GAOJIMicrosoft" in req.text:
                printGreen("[+]Success:存在xplus MSSQL通用注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在xplus_mysql_mssql_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = xplus_mysql_mssql_sqli(sys.argv[1])
    testVuln.run()