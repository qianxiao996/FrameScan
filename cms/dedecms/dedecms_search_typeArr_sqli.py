...
class dedecms_search_typeArr_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if r"Error infos" in req.text and r"Error sql" in req.text:
                printGreen("[+]Success:存在dedecms search.php SQL注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在dedecms_search_typeArr_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = dedecms_search_typeArr_sqli(sys.argv[1])
    testVuln.run()