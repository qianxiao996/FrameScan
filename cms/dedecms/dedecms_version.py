...
class dedecms_version:
    ...
    def run(self):
        ...
        try:
            ...
            if m:
                printGreen("[+]探测到dedecms版本...(敏感信息)\t时间戳: %s, 版本信息: %s"%(m.group(1), self.check_ver(m.group(1))))
            else:
                printBlue("[-]Info:不存在dedecms_version漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = dedecms_version(sys.argv[1])
    testVuln.run()