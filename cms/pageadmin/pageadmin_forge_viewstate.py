...
class pageadmin_forge_viewstate:
    ...
    def run(self):
        ...
        try:
            ...
            if req.status_code == 200 and r"WebForm_DoPostBackWithOptions" in req.text and r"Tb_sql" in req.text:
                printGreen("[+]Success:存在PageAdmin可“伪造”VIEWSTATE执行任意SQL查询&重置管理员密码漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在pageadmin_forge_viewstate漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = pageadmin_forge_viewstate(sys.argv[1])
    testVuln.run()