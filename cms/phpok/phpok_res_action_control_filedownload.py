...
class phpok_res_action_control_filedownload:
    ...
    def run(self):
        ...
        try:
            ...
            if r"<?php" in req.text and r"host" in req.text:
                printGreen("[+]Success:存在phpok res_action_control.php 任意文件下载(需要cookies文件)漏洞...(高危)\tpayload: "+vulnurl+"\ncookies:"+json.dumps(cookies, indent=4))
            else:
                printBlue("[-]Info:不存在phpok_res_action_control_filedownload漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = phpok_res_action_control_filedownload(sys.argv[1])
    testVuln.run()