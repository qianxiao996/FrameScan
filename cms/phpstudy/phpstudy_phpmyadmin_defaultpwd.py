...
class phpstudy_phpmyadmin_defaultpwd:
    ...
    def run(self):
        ...
        try:
            ...
            if r"navigation.php" in req2.text and r"frame_navigation" in req.text:
                printGreen("[+]Success:存在phpstudy phpmyadmin默认密码漏洞...(高危)\tpayload: "+vulnurl+"\tpost: "+json.dumps(post_data, indent=4))
            else:
                printBlue("[-]Info:不存在phpstudy_phpmyadmin_defaultpwd漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = phpstudy_phpmyadmin_defaultpwd(sys.argv[1])
    testVuln.run()
