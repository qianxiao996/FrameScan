    ...
    def run(self):
        ...
        try:
            ...
            if r"button_normal" in req.text:
                printGreen("[+]Success:存在璐华企业版OA系统POST SQL注入漏洞...(高危)\tpayload: "+self.url+"/include/get_user.aspx")
                noexist = False
            if noexist:
                printBlue("[-]Info:不存在ruvar_oa_multi_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = ruvar_oa_multi_sqli(sys.argv[1])
    testVuln.run()