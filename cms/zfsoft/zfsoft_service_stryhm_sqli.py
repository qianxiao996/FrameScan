...
class zfsoft_service_stryhm_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if res_true!=res_false:
                printGreen("[+]Success:存在正方教务系统services.asmx SQL注入漏洞...(高危)\tpayload: "+vulnurl+"..[需要对比查看xml文件内容]")
            else:
                printBlue("[-]Info:不存在zfsoft_service_stryhm_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = zfsoft_service_stryhm_sqli(sys.argv[1])
    testVuln.run()