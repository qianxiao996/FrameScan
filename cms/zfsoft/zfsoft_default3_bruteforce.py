...
class zfsoft_default3_bruteforce:
    ...
    def run(self):
        ...
        try:
            ...
            if r"__VIEWSTATEGENERATOR" in req.text and r"CheckCode.aspx" not in req.text and req.status_code ==200:
                printGreen("[+]Success:存在正方教务系统default3.aspx爆破页面...(敏感信息)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在zfsoft_default3_bruteforce漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = zfsoft_default3_bruteforce(sys.argv[1])
    testVuln.run()
