...
class kxmail_login_server_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if time.time() - start_time >= 6:
                printGreen("[+]Success:存在科信邮件系统login.server.php 时间盲注漏洞...(高危)\tpayload: "+vulnurl+"\npost: "+json.dumps(post_data, indent=4))
            else:
                printBlue("[-]Info:不存在kxmail_login_server_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = kxmail_login_server_sqli(sys.argv[1])
    testVuln.run()