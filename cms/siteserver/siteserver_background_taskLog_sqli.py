...
class siteserver_background_taskLog_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if req.status_code == 500 and r"Microsoft" in req.text:
                printGreen("[+]Success:存在siteserver3.6.4 background_taskLog.aspx注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在siteserver_background_taskLog_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = siteserver_background_taskLog_sqli(sys.argv[1])
    testVuln.run()
