...
class siteserver_UserNameCollection_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if r"BBBMicrosoft" in req.text:
                printGreen("[+]Success:存在siteserver3.6.4 user.aspx注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在siteserver_UserNameCollection_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = siteserver_UserNameCollection_sqli(sys.argv[1])
    testVuln.run()