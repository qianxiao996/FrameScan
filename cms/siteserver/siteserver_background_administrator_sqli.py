...
class siteserver_background_administrator_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if r"BBBMicrosoft" in req.text:
                printGreen("[+]Success:存在siteserver3.6.4 background_administrator.aspx注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在siteserver_background_administrator_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = siteserver_background_administrator_sqli(sys.argv[1])
    testVuln.run()
