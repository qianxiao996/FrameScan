    ...
    def run(self):
        ...
        try:
            ...
            if r"Configuration File (php.ini) Path" in req.text:
                printGreen("[+]Success:存在TPshop eval-stdin.php 代码执行漏洞...(高危)\tpayload: "+vulnurl+"\tpost: "+json.dumps(post_data, indent=4))
            else:
                printBlue("[-]Info:不存在tpshop_eval_stdin_code_exec漏洞")
