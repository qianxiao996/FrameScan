    ...
    def run(self):
        ...
        try:
            ...
            if r"Active Internet connections" in req.text:
                printGreen("[+]Success:存在rap接口struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Linux]")

            elif r"Active Connections" in req.text or r"活动连接" in req.text:
                printGreen("[+]Success:存在rap接口struts 命令执行漏洞...(高危)\tpayload: "+vulnurl+"\t[Windows]")

            elif r"LISTEN" in req.text:
                cprint("[+]可能存在rap接口struts 命令执行漏洞...(高危)\tpayload: "+vulnurl)

            else:
                printBlue("[-]Info:不存在rap_interface_struts_exec漏洞")
