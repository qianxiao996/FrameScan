        ...
        try:
            for turl in urls:
                ...
                if r"GAO JI@Microsoft" in req.text:
                    printGreen("[+]Success:存在金窗教务系统存在多处SQL注射漏洞...(高危)\tpayload: "+vulnurl)
                    noexist = False
            if noexist:
                printBlue("[-]Info:不存在gowinsoft_jw_multi_sqli漏洞")
        except:
            printYellow("[-]Warning:"+self.__class__.__name__+" ==>可能不存在漏洞")

if __name__ == "__main__":
    warnings.filterwarnings("ignore")
    testVuln = gowinsoft_jw_multi_sqli(sys.argv[1])
    testVuln.run()